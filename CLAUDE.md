...
**公開Web Playerの匿名API経路を使う。** JPでは2026-09-11の実通信で曲・アルバム・
...
discoveryだけに使い、カタログデータはAPI responseから取得する。検索はdesktop Webの
`showSearch` BFFとして実装し、GraphQLやmobile用Tenzing RESTと混同しない。ID引きはlive確認済みの
GraphQL operationとfieldだけをproviderへ接続する。
