...
このプラグインは Amazon Music API を**直接は叩かない**。認証を担う自前のバックエンド
...
Amazon Music API（`https://api.music.amazon.com`）はすべてのリクエストに
...
`api.music.amazon.com` に中継し、**レスポンスボディをそのまま返す**。整形しない。

| メソッド | パス | 用途 |
| --- | --- | --- |
| GET | `/v1/catalog/{marketplace}/search` | 曲・アルバム・アーティストの検索 |
| GET | `/v1/catalog/{marketplace}/songs/{id}` | 曲の ID 引き |
| GET | `/v1/catalog/{marketplace}/albums/{id}` | アルバムの ID 引き |
| GET | `/v1/catalog/{marketplace}/artists/{id}` | アーティストの ID 引き |

`{marketplace}` は `jp` または `us`（プラグインの設定で決まる。将来他の地域も渡りうる
