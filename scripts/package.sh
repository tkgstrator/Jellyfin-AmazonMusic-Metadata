#!/usr/bin/env bash
# Build release artifacts for every supported Jellyfin ABI.
#
#   ./scripts/package.sh [version]
#
# Produces, for each ABI, an installable plugin archive under dist/:
#   dist/<abi-name>/                                 unpacked plugin directory
#   dist/amazon-music_<version>_<abi-name>.zip        archive
#   dist/amazon-music_<version>_<abi-name>.zip.md5    checksum (Jellyfin manifests use MD5)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

PROJECT="Jellyfin.Plugin.AmazonMusic/Jellyfin.Plugin.AmazonMusic.csproj"
DIST="$ROOT/dist"

# (target-framework, jellyfin-abi, archive-suffix)
TARGETS=(
  "net9.0:10.11.0.0:jellyfin-10.11"
...
for target in "${TARGETS[@]}"; do
  ...
  find "$outdir" -type f \
    ! -name 'Jellyfin.Plugin.AmazonMusic.dll' \
    ! -name 'Jellyfin.Plugin.AmazonMusic.pdb' \
    -delete
  ...
  archive="$DIST/amazon-music_${VERSION}_${name}.zip"
